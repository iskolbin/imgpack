static int imgpack_formatter_CSV(struct ImgPackContext *ctx, FILE *f) {
	...
	for (int i = 0; i < ctx->size; i++) {
		...
		fprintf(f, "%d, \"%s\", %d, %d, %d, %d, %d, %d, \"%s\", \"%s\", \"%.5g\"\n", i, ctx->imagePaths[i], frame.x, frame.y, frame.w, frame.h, ctx->sourceRects[i].w, ctx->sourceRects[i].h, ctx->outputImagePath, get_output_image_format(ctx), ((1.0*ctx->scaleNumerator)/ctx->scaleDenominator));
	}
	return 0;
}
